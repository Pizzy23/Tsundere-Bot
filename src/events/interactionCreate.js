...
module.exports = (client, int) => {
    ...
    if (int.isButton()) {
        ...
        if (!queue || !queue.isPlaying())
            return int.reply({ content: `❌ | Não tem barulho no momento..`, ephemeral: true, components: [] });
        ...
        switch (int.customId) {
            case 'Save Song': {
                ...
                if (!queue || !queue.isPlaying())
                    return int.reply({ content: `❌ | Nenhum barulho tocando.`, ephemeral: true, components: [] });
